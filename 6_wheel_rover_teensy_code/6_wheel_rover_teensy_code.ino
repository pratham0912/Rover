...
// Functions 
void setupMotors() { // setting enable true ==
...
void stopMotors() { // stopping all motors 
  motorLF.stop();
  motorLM.stop();
  motorLR.stop();
  motorRF.stop();
...
void setLeftMotors(int speed) {// Left Motor speed setting
  motorLF.rotate(speed);
  motorLM.rotate(speed);
  motorLR.rotate(speed);
}

void setRightMotors(int speed) {// Right motor speed setting
  motorRF.rotate(speed);
  motorRM.rotate(speed);
  motorRR.rotate(speed);
}

void receiveEvent(int byte_received) {// I2C receive
  if (Wire.available() >= 2) {
    X = Wire.read();
    Y = Wire.read();
